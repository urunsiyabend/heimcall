...
Last updated: 2026-06-19 (Sprint 21 - Phase 13 T1: test coverage — incident-service domain unit tests (18, pure Mockito, no infra). Infra decision locked: Testcontainers unusable on this box; the strategy is Mockito + EmbeddedKafka + compose-PG)
...
| Test coverage thin (Phase 13 in progress): T1 DONE — incident-service domain units (18, Mockito). Remaining: incident Kafka resilience/EmbeddedKafka (T2), escalation (T3), notification (T4), integration + schedule edge (T5). No Testcontainers (unusable on this box); no full-stack containerized e2e (live-fleet manual run stays the e2e gate) | Phase 13 |
