...
Last updated: 2026-06-19 (Sprint 24 - Phase 13 T4: test coverage — notification-service. `DeliveryServiceTest` (7, Mockito): retry/backoff decision + terminal FAILED + guards. `NotificationServiceTest` (3): enabled-contact fan-out. `NotificationDeliveryClaimTest` (1, compose-PG): FOR UPDATE SKIP LOCKED no-double-send)
...
| Test coverage thin (Phase 13 in progress): T1 DONE — incident-service domain units (18, Mockito); T2 DONE — incident Kafka resilience (4, `@EmbeddedKafka`); T3 DONE — escalation (12: materialization/guards/cancel/fireDueTask Mockito + claim compose-PG); T4 DONE — notification (11: retry/backoff + fan-out Mockito + claim compose-PG). Remaining: integration + schedule edge (T5). No Testcontainers (unusable on this box); no full-stack containerized e2e (live-fleet manual run stays the e2e gate) | Phase 13 |
