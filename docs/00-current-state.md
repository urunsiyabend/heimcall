...
Last updated: 2026-06-19 (Sprint 22 - Phase 13 T2: test coverage — incident-service Kafka resilience over `@EmbeddedKafka` (4 tests, in-JVM broker, no Docker/DB): DLT routing for poison-pill + application-exception, plus type-header feedback dispatch)
...
| Test coverage thin (Phase 13 in progress): T1 DONE — incident-service domain units (18, Mockito); T2 DONE — incident Kafka resilience (4, `@EmbeddedKafka`, DLT routing + feedback dispatch). Remaining: escalation (T3), notification (T4), integration + schedule edge (T5). No Testcontainers (unusable on this box); no full-stack containerized e2e (live-fleet manual run stays the e2e gate) | Phase 13 |
