...
Last updated: 2026-06-20 (Sprint 26 - Phase 14 T1: Redis activation — rate limit the integration ingest route at the gateway with a Redis token bucket keyed per integrationKey. First real Redis wiring; over-limit → 429, fail-open if Redis down.)
...
| ~~Redis in compose but wired into zero services~~ PARTIALLY DONE Phase 14: gateway rate limit (T1) is the first use; notification cooldown (T2) next. Locks/idempotency-cache/on-call-cache deliberately not on Redis (regress/redundant/low-value) | Phase 14 |
...
| Notification preference is just per-contact-method `enabled`; no cooldown / per-incident throttle / quiet hours | Phase 14 T2 (cooldown) / later |
| Contact-method `destination` not format-validated (email/url); no verification step | later |
| No Redis notification-cooldown cache | Phase 14 T2 |
